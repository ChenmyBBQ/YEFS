...
3. 点击在地图上工具位置，不记录位置，点击在地图上，才算有效数据。控制器不直接下点，而是先发送 `airspace-manager/draw` 启动插件绘制状态。
4. 插件进入 `Drawing` 状态后，接收后续在地图上点击的位置信息。
5. 每次点击调用 `DrawingController::addPoint()` 累积控制点。
6. 鼠标移动时，插件用 hover 点生成预览几何。
7. 当点数大于1个以后，右侧弹出属性编辑框，显示图形属性，可以双击完成，或者属性页中点击确认或者应用（属性页中有三个按钮，确认/应用、取消）。根据不同类型图形，实时更新属性。
